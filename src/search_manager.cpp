...
std::string SearchManager::SearchNSendNewComputerGui(std::string file_name, 
                        std::shared_ptr<remote_tasker::Socket> sock)
{
    std::ifstream file;
                        ...
                        std::shared_ptr<remote_tasker::Socket> sock)
{
    ...
    if(*size == 0)
    {
        return "the file: " + file_name + " not found";    
    }
    ...
    if (file_data.size() == 1 && file_data.front() == Socket::BAD_VECTOR)
    {
        ...
        return "there was a problem with the file, abort save.";
    }
    ...
    return "the file: " + file_name + " saved into the " + m_save_to 
    + " directory ";     
}
